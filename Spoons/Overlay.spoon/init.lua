...
Overlay.version = "1.6"
Overlay.author = "{ ken }"
Overlay.homepage = "https://github.com/kenbankspeng/hammerspoon-config"
Overlay.license = "MIT - https://opensource.org/licenses/MIT"
...
local function areFramesEqual(frame1, frame2)
...
local function createCanvasElement(message, frame)
...
local function findTextLayer(canvas)
  for i = #canvas, 1, -1 do
...
local function updateCanvas(message, frame, isNewSpace)
  if not canvas or isNewSpace then
    if canvas then canvas:hide() end
    canvas = createCanvasElement(message, frame)
  else
    canvas:frame(frame)
    local textElement = findTextLayer(canvas)
    ...
    if textElement.text ~= message or not areFramesEqual(textElement.frame, newTextFrame) then
...
local function pollActiveWindow()
  ...
  if window then
    ...
    local hasFrameChanged = not previousFrame or not areFramesEqual(frame, previousFrame)
    ...
    if hasWindowChanged or hasFrameChanged or hasSpaceChanged then
      ...
      if app then
        ...
        updateCanvas(appName, frame, hasSpaceChanged)
      end
    end
  else
...
function Overlay:toggle()
  if isEnabled then
    ...
    if not timer then
      timer = hs.timer.doEvery(config.pollInterval, pollActiveWindow)
    else
      timer:start()
    end
    isEnabled = true
...
function Overlay:bindHotkey(modifiers, key)
  hs.hotkey.bind(modifiers, key, function() self:toggle() end)
end

-- Start the highlight with custom configuration
function Overlay:start(customConfig, customAppNames)
