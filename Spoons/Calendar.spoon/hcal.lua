return function(param)
  ...
  hcal.timer                    = nil
  hcal.displayDate              = os.date("*t")
  hcal.screen                   = hs.screen.mainScreen():fullFrame()

  hcal.canvas                   = hs.canvas.new({
    x = param.canvas.margin.left,
    y = hcal.screen.h - param.canvas.height - param.canvas.margin.bottom,
  ...
  hcal.helpers                  = dofile(hs.spoons.resourcePath("helpers.lua"))(hcal.canvas)
  local canvasIndex             = hcal.helpers.canvasIndex
  local canvasEl                = hcal.helpers.canvasEl
  local place                   = hcal.helpers.place
  local scale                   = hcal.helpers.scale
  local daysInMonth             = hcal.helpers.daysInMonth
  local calculateDateToday      = hcal.helpers.calculateDateToday
  local calculateMonthYear      = hcal.helpers.calculateMonthYear
  local calculateDowForFirstDom = hcal.helpers.calculateDowForFirstDom
  local weeknames               = hcal.helpers.weeknames


  function hcal:updateCanvas()
    ...
    local showTodayIndicator = givenYear == dateToday.year and givenMonth == dateToday.month and
        givenDay == dateToday.day
    ...
    canvasEl("title").text   = calculateMonthYear(hcal.displayDate)
    ...
    for i = 1, 31 do
      ...
      if i <= daysInGivenMonth then
        ...
        if showTodayIndicator and i == givenDay then
          ...
          canvasEl("todayIndicator").frame.x = day1x + (i - 1) * dayWidth +
  ...
  function hcal:createCanvas()
    ...
    self.canvas:appendElements({
      ...
      strokeColor = param.selector.button.strokeColor,
      strokeWidth = param.selector.button.strokeWidth,
      coordinates = place(param.selector.button.prev.x, param.selector.button.prev.y,
    ...
    self.canvas:appendElements({
      ...
      strokeColor = param.selector.button.strokeColor,
      strokeWidth = param.selector.button.strokeWidth,
      coordinates = place(param.selector.button.next.x, param.selector.button.next.y,
    ...
    self.canvas:mouseCallback(function(canvas, event, id, x, y)
      if event == "mouseDown" then
        if id == "nextMonth" then self:nextMonth() end
        if id == "prevMonth" then self:prevMonth() end
      end
    end)
  end

  function hcal:showCalendar()
    if self.canvas then self.canvas:show() end
  end

  function hcal:hideCalendar()
    if self.canvas then self.canvas:hide() end
  end

  function hcal:showToday()
    self.displayDate = os.date("*t")
    self:updateCanvas()
  end

  function hcal:prevMonth()
